...
const NavLink: React.FC<{ href: string; children: React.ReactNode; onClick?: () => void; }> = ({ href, children, onClick }) => (
  <a href={href} onClick={onClick} className="text-foreground-light dark:text-foreground-dark hover:text-primary-DEFAULT dark:hover:text-primary-DEFAULT transition-colors duration-300 font-medium">
    {children}
  </a>
);
    ...
    <header id="header" className={`sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-lg shadow-md' : 'bg-transparent'}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center space-x-4">
            ...
            <a href="/memberships" className="hidden sm:inline-block bg-primary hover:bg-primary-dark text-white font-bold py-2 px-6 rounded-xl transition-transform duration-300 ease-in-out transform hover:scale-105">
              Únete Ahora
            </a>
      ...
      <div className={`lg:hidden ${isOpen ? 'block' : 'hidden'} bg-background-light dark:bg-background-dark absolute w-full shadow-lg`}>
        <nav className="flex flex-col items-center space-y-4 py-6">
          ...
          <a href="/memberships" onClick={() => setIsOpen(false)} className="bg-primary hover:bg-primary-dark text-white font-bold py-3 px-8 rounded-xl transform hover:scale-105 w-full text-center max-w-xs">
            Únete Ahora
          </a>
        </nav>
      </div>
